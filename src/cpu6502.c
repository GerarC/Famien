...
void initialize_cpu(Cpu6502 *cpu) {
	...
	Instruction instructions[] = {
		...
		{"???", &XXX, &IMP, 3}, {"STY", &STY, &ABS, 4}, {"STA", &STA, &ABS, 4},
		{"STX", &STX, &ABS, 4}, {"???", &XXX, &IMP, 4}, {"BCC", &BCC, &REL, 2},
		{"STA", &STA, &IZY, 6}, {"???", &XXX, &IMP, 2}, {"???", &XXX, &IMP, 6},
		{"STY", &STY, &ZPX, 4}, {"STA", &STA, &ZPX, 4}, {"STX", &STX, &ZPY, 4},
		{"???", &XXX, &IMP, 4}, {"DEY", &DEY, &IMP, 2}, {"???", &NOP, &IMP, 2},
		{"TXA", &TXA, &IMP, 2}, {"???", &XXX, &IMP, 2}, {"TYA", &TYA, &IMP, 2},
...
void connect_bus(Cpu6502 *cpu, Bus *bus) { cpu->bus = bus; }

void connect_bus_methods(Cpu6502 *cpu, void (*write_ram)(Bus *, Word, Byte),
						 Byte (*read_ram)(Bus *, Word)) {
...
char *parse_cpu_str(Cpu6502 cpu) {
	...
	snprintf(buffer, sizeof(buffer),
			 "Cpu {pc: %04X, sp: %04X, a: %02X, x: %02X, y: %02X, flags: %02X, "
			 "opcode: %02X, cycles: %i, addr_abs: %04X}",
			 cpu.pc, cpu.sp, cpu.a, cpu.x, cpu.y, cpu.st, cpu.opcode,
			 cpu.cycles, cpu.address_absolute);
	return buffer;
}
...
Byte ABX(Cpu6502 *cpu) {
	...
	if ((cpu->address_absolute & 0xFF00) != (hi << 8))
		return 1;
	return 0;
}
...
Byte IND(Cpu6502 *cpu) {
	...
	if (ptr_lo == 0x00FF)
		cpu->address_absolute = (cpu->read_ram(cpu->bus, ptr & 0xFF00) << 8 |
								 cpu->read_ram(cpu->bus, ptr + 0));
	else
		cpu->address_absolute = (cpu->read_ram(cpu->bus, ptr + 1) << 8 |
								 cpu->read_ram(cpu->bus, ptr + 0));
	return 0;
}
...
Byte IZY(Cpu6502 *cpu) {
	...
	if ((cpu->address_absolute & 0xFF00) != (hi << 0))
		return 1;
	return 0;
}
...
Byte SBC(Cpu6502 *cpu) {
	...
	set_flag(cpu, Z, (cpu->temp & 0x00FF) == 0x0000);
	set_flag(cpu, V, (cpu->temp ^ (Word)cpu->a) & (cpu->temp ^ value) & 0x0080);
	set_flag(cpu, N, cpu->temp & 0x80);
	cpu->a = cpu->temp & 0x00FF;
	return 1;
}
...
Byte BCC(Cpu6502 *cpu) {
	if (cpu->c == 0) {
		...
		if ((cpu->address_absolute & 0xFF00) != (cpu->pc & 0xFF00))
			cpu->cycles++;
